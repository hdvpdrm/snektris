...
class FancyText
	...
	, public sf::Drawable {
public:
	explicit FancyText(sf::VertexBuffer::Usage usage = sf::VertexBuffer::Static);
...
struct line_spacing_t {
	float factor;
};
struct color_t {
	sf::Color color;
};
struct outline_color_t {
	sf::Color color;
};
struct outline_thickness_t {
	float thickness;
};
...
sf::Text::Style styles(Ts... ts) {
	return static_cast<sf::Text::Style>((sf::Text::Style::Regular | ... | sf::Text::Style{ ts }));
}
...
class TextBuilder {
public:
	explicit TextBuilder(sf::Font const& font_);

	friend TextBuilder& operator<<(TextBuilder& builder, sf::Font const& font);
	...
	void set_font(sf::Font const& font_);
...
private:
	...
	struct CharacterSizeChanged {
		sf::Texture const* texture;
		std::size_t character_position;
	};
